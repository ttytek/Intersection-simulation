...
Position lights_in_order[12]= {
	{0,3},{0,2},{0,1},{1,0},{1,3},{1,2},{3,0},{3,1},{3,0},{2,3},{2,0},{2,1}
};
...
void draw(simulation* sim){
    ...
    for(int i=0;i<4;i++){
        for(int j=0;j<4;j++){
			if(i==j)	continue; //not implemented
    ...
    for(int i=0;i<HEIGHT;i++){
		for(int j=0;j<strlen(map[i]);j++){
			if(map[i][j]=='C'){
				state light = sim->lights[lights_in_order[n].col][lights_in_order[n].row];
				n++;
				char* color;
				switch(light){
					case GREEN:
	...
	for(int i=0;i<4;i++){
		for(int j=0;j<4;j++){
			if(i!=j){
				...
				if(map[col][row]!='C'){
					dprintf(2,"at %d %d expected %c got %c\n",col,row,'C',map[col][row]);
				}
				if(map[col][row+2]!='R'){
					dprintf(2,"at %d %d expected %c got %c\n",col,row+2,'R',map[col][row+2]);
				}
			}
		}
	}
